...
CREATE OR REPLACE PACKAGE BODY application_user_pkg IS
    PROCEDURE insert_all(
        ...
        -- Exception for username uniqueness violation
        username_already_exists EXCEPTION;
        PRAGMA EXCEPTION_INIT(username_already_exists, -00001); -- Assuming -00001 as the Oracle error for unique constraint violation
        
        -- Exception for pincode length violation
    ...
    BEGIN
        ...
        -- Check for pincode length (assuming pincode length should be exactly 5 characters)
        ...
        VALUES (p_user_name, p_first_name, p_last_name, p_email_id, p_password, p_phone_number, 
                p_date_of_birth, p_gender, SYSDATE, 'A', p_role)
    ...
    EXCEPTION
        ...
        WHEN invalid_pincode THEN
            dbms_output.put_line('Error: Pincode must be exactly 5 characters.');
...
BEGIN
    application_user_pkg.insert_all('user1', 'John', 'Doe', 'john.doe@example.com', 'password123', '555-0100', 
                                    TO_DATE('1985-01-01', 'YYYY-MM-DD'), 'Male', 'Customer', 
                                    '100 Beacon St', 'Apt 1', 'Boston', 'MA', 'USA','02109', 42.3557, -71.0723, 
                                    'Driving License', '/path/to/driving/license/user1.jpg');
                                    
    application_user_pkg.insert_all('user2', 'Jane', 'Smith', 'jane.smith@example.com', 'password456', '555-0101', 
                                    TO_DATE('1990-02-02', 'YYYY-MM-DD'), 'Female', 'Customer', 
                                    '200 Clarendon St', 'Floor 25', 'Boston', 'MA', 'USA','02116', 42.3494, -71.0759, 
                                    'SSN', '/path/to/ssn/user2.jpg');
                                    
    application_user_pkg.insert_all('user3', 'Emily', 'Jones', 'emily.jones@example.com', 'password789', '555-0102', 
                                    TO_DATE('1995-03-03', 'YYYY-MM-DD'), 'Female', 'Customer', 
                                    '150 Dorchester Ave', 'Suite 5', 'Boston', 'MA', 'USA','02127', 42.3412, -71.0567, 
                                    'Driving License', '/path/to/driving/license/user3.jpg');
                                    
    application_user_pkg.insert_all('user4', 'Michael', 'Brown', 'michael.brown@example.com', 'password101', '555-0103', 
                                    TO_DATE('1988-04-04', 'YYYY-MM-DD'), 'Male', 'Customer', 
                                    '101 Seaport Blvd', 'Apt 15', 'Boston', 'MA', 'USA','02210', 42.3495, -71.0435, 
                                    'SSN', '/path/to/ssn/user4.jpg');
                                    
    application_user_pkg.insert_all('user5', 'Linda', 'Taylor', 'linda.taylor@example.com', 'password202', '555-0104', 
                                    TO_DATE('1992-05-05', 'YYYY-MM-DD'), 'Female', 'Customer', 
